...
const h = (b: string, index: number) => {
  const cc = b.charCodeAt(index);
  return HEXMAP[cc >> 4]! + HEXMAP[cc & 15]!;
};

const hb = (c: number) => (c <= CC9 ? c - CC0 : c - CCa_offset10);

const b = (h: string, i: number) =>
  String.fromCharCode((hb(h.charCodeAt(i)) << 4) | hb(h.charCodeAt(i + 1)));

export const toBase64UUID = (u: string): string => {
...
export const fromBase64UUID = (cuuid: string): string => {
  let base64 = "";
  let lastI = 0;
