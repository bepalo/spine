...
export const parseCookieFromRequest = <Expected extends Record<string, string>>(
  ...
  if (cookieHeader != null) {
    const cookie: Record<string, string | unknown> = cookieOut ?? {};
    // 1: cookie-name
    // 2: cookie-value
    let mode = 1;
    let key;
      ...
      if (cc < 33 || cc > 126) {
        ...
        if (mode === 1) {
          throw new HttpError(
            ...
            "Invalid cookie-name token",
          );
        } else if (mode === 2) {
          throw new HttpError(
            Status._400_BadRequest,
            "Invalid cookie-value token",
          );
        }
      }
      switch (cc) {
        case 59: // ";"
          if (mode === 2) {
            ...
            if (dQuoteI > -1) {
              ...
              throw new HttpError(
                Status._400_BadRequest,
                "Invalid cookie-separator token",
              );
            }
            cookie[key!] = value;
            mode = 1;
    ...
    if (lastI <= i) {
      if (mode === 2 && dQuoteI < 0) {
        ...
        throw new HttpError(Status._400_BadRequest, "Invalid cookie");
      }
    }
    return cookie as Expected;
  }
  return undefined;
};
