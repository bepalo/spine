...
export type HttpMethod = "Head" | "Get" | "Query" | "Post" | "Put" | "Patch" | "Delete" | "Options" | "Trace" | "Connect";
export type HttpMethodLower = "head" | "get" | "query" | "post" | "put" | "patch" | "delete" | "options" | "trace" | "connect";
export type HttpMethodUpper = "HEAD" | "GET" | "QUERY" | "POST" | "PUT" | "PATCH" | "DELETE" | "OPTIONS" | "TRACE" | "CONNECT";
...
export type ExtractPathFromMethodPath<MP extends MethodPath> = MP extends `${HttpMethod} ${infer Path}` ? Path : never;
/**
 ...
 */
export type ExtractMethodPathsParams<MP extends string> = MP extends `${HttpMethod} ${infer Path}` ? ExtractParamName<SplitPathSegments<Path>[number]> : never;
...
export type DefaultHandler<ExtendContext extends Record<string, unknown> = Record<string, never>> = (ctx: Context<ExtendContext>) => Promise<DefaultHandlerReturn> | DefaultHandlerReturn;
export type EndHandler<ExtendContext extends Record<string, unknown> = Record<string, never>> = (ctx: Context<Readonly<CTResponse> & ExtendContext>) => Promise<void> | void;
export type DefaultEndHandler<ExtendContext extends Record<string, unknown> = Record<string, never>> = (ctx: Context<Readonly<CTResponse> & ExtendContext>) => Promise<void> | void;
export type DefaultEndErrorHandler<ExtendContext extends Record<string, unknown> = Record<string, never>> = (ctx: Context<CTError & Readonly<CTResponse> & ExtendContext>) => Promise<void> | void;
export type EndPipe<ExtendContext extends Record<string, unknown> = Record<string, never>> = Array<EndHandler<Readonly<CTResponse> & ExtendContext>>;
